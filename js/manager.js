...
Component.entryPoint = function(NS){
	...
	NS.data = NS.data || new Brick.util.data.byid.DataSet('template');
	var DATA = NS.data;
	...
	TemplateWidget.prototype = {
		init: function(container){
			var TM = buildTemplate(this, 'panel,table,row,rowwait,rowdel,rowgrptype0,rowgrptype1,rowgrptype2,rownmedit');
			...
			if (DATA.isFill(this.tables)){
				...
				TM.getEl("panel.table").innerHTML = TM.replace('table', {'rows': TM.replace('rowwait')});
			}
		},
	...
	YAHOO.extend(Editor, Brick.widget.Dialog, {
		initTemplate: function(){
			var TM = buildTemplate(this, 'editor,param,tableparam,rowparam,option,parameditor');
	...
	Param.prototype = {
		init: function(container, obj){
			...
			var TM = buildTemplate(this, 'param,tableparam,rowparam');
			
			container.innerHTML = TM.replace('param');
			this.brickid = obj['id'];
		...
		render: function(){
			...
			TM.getEl('param.table').innerHTML = TM.replace('tableparam', {'rows': lst});
		},
		onClick: function(el){
			var TId = this._TId;
	...
	YAHOO.extend(ParamEditor, Brick.widget.Dialog, {
		initTemplate: function(){
			var TM = buildTemplate(this, 'parameditor');
			return TM.replace('parameditor');
		},
		onLoad: function(){
		...
		el: function(name){ return Dom.get(this._TId['parameditor'][name]); },
		elv: function(name){ return Brick.util.Form.getValue(this.el(name)); },
		setelv: function(name, value){ Brick.util.Form.setValue(this.el(name), value); },
		onClick: function(el){
			var tp = this._TId['parameditor']; 
			switch(el.id){
			case tp['bcancel']: this.close(); return true;
			case tp['bsave']: this.save(); return true;
			}
	...
	NS.API.showTemplateWidget = function(container){
		new NS.TemplateWidget(container);
